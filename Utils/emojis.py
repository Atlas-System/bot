        ...
        for emoji in self.init_emojis:
            if emoji not in [e.name for e in emojis]:
                with open(f"Assets/Emoji/{emoji}.png", "rb") as image_file:
                    ...
                    self.emojis[emoji] = f"<:{created_emoji.name}:{created_emoji.id}>"
                ...
                self.emojis[emoji] = f"<:{existing_emoji.name}:{existing_emoji.id}>"

        self.bot.logger.info(f"Application Emojis Setup. {self.emojis}")
